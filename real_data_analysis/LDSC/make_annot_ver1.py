...
def make_annot_files(args, bed_for_annot):
    ...
    pd.DataFrame(np.array(iter_bim)).to_csv('temp.bed',sep='\t',index=False,header=False)
    bimbed = BedTool('temp.bed')
    ...
    #df_annot.to_csv(args.annot_file.split('.gz')[0]+'.csv',index=False)
    df_annot.fillna(0, inplace=True)
    str_index = args.annot_file.find('annot')
    out_dir = args.annot_file[0:str_index+5]+'_overlap/'+args.annot_file.split('/')[-1]
    with gzip.open(out_dir, 'wb') as g:
        df_annot.to_csv(g, sep = "\t", index = False)
